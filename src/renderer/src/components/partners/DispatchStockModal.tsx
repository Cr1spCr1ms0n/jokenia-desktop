...
import type { DispatchLine, PartnerVariationOption } from './types'

const SEARCH_DEBOUNCE_MS = 300

function blankLine(): DispatchLine {
  return { variation: null, quantity: '', selling_price: '' }
}

// No branding filter here (unlike Consignees' Receive Stock) — the admin
// app's own dispatch screen searches the general product cache with no
// per-partner scoping, since partner stock isn't tied to a branded catalog.
async function searchVariations(query: string): Promise<PartnerVariationOption[]> {
...
interface LineEditorProps {
  ...
  index: number
  canRemove: boolean
  onChange: (patch: Partial<DispatchLine>) => void
  onRemove: () => void
}

function LineEditor({ line, index, canRemove, onChange, onRemove }: LineEditorProps): React.JSX.Element {
  const [query, setQuery] = useState('')
  const [results, setResults] = useState<PartnerVariationOption[]>([])
  ...
  return (
    <div className="space-y-2 rounded-md border border-jokenia-tan/20 bg-white/70 p-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-wide text-jokenia-tan">Line {index + 1}</p>
        {canRemove && (
      ...
      {line.variation ? (
        ...
        <div className="relative">
          ...
          {!searching && query.trim().length >= 2 && results.length === 0 && (
            <p className="mt-1 text-xs text-jokenia-tan">No variations match &quot;{query}&quot;.</p>
          )}
        </div>
      )}
      ...
      <p className="text-[11px] italic text-jokenia-tan">Price to sell at (set by the business)</p>
    </div>
  )
}
...
function DispatchStockModal({
  ...
  onClose,
  onDispatched
}: DispatchStockModalProps): React.JSX.Element {
  const [dispatchDate, setDispatchDate] = useState(() => new Date().toISOString().slice(0, 10))
  const [notes, setNotes] = useState('')
  const [lines, setLines] = useState<DispatchLine[]>([blankLine()])
  const [formError, setFormError] = useState<string | null>(null)

  const dispatchMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.rpc('dispatch_to_partner', {
        ...
        p_lines: lines.map((line) => ({
          ...
          selling_price: parseFloat(line.selling_price)
        }))
      })
  ...
  function updateLine(index: number, patch: Partial<DispatchLine>): void {
  ...
  function handleSubmit(): void {
    if (dispatchDate > new Date().toISOString().slice(0, 10)) {
      setFormError('Dispatch date cannot be in the future.')
      return
    }
    for (const line of lines) {
      ...
      if (!(parseFloat(line.selling_price) > 0)) {
        setFormError('Enter a selling price greater than zero for every line.')
        return
      }
    }
    setFormError(null)
  ...
  return (
    <Modal title="Dispatch stock" isOpen={isOpen} onClose={onClose} maxWidthClassName="max-w-lg">
      <div className="max-h-[70vh] space-y-3 overflow-y-auto">
        {(formError || dispatchMutation.error) && (
          <p className="text-xs text-red-500">{formError ?? (dispatchMutation.error as Error).message}</p>
        )}
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleSubmit} disabled={dispatchMutation.isPending} className="flex-1">
